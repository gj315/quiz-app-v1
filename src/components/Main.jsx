...
class Main extends React.Component {
  ...
  pushData (nr) {
    ...
    const getCorrectOptionIndex = getSuffledOptions.indexOf(decodeURIComponent(dataSet[nr].correct_answer))
    const difficulty = this.getQuesDifficultyLevel(decodeURIComponent(dataSet[nr].difficulty))
  ...
  handleCorrectAttemp () {
    let { totalQuestionCorrect, totalQuestionAttempted, score, total } = this.state
    score = score + 1
    totalQuestionAttempted = totalQuestionAttempted + 1
    totalQuestionCorrect = totalQuestionCorrect + 1

    const totalPercentage = this.handlePercentage(totalQuestionCorrect, totalQuestionAttempted, total)
  ...
  handlePercentage (totalQuestionCorrect, totalQuestionAttempted, total) {
    const currentScorePercentage = parseInt((totalQuestionCorrect / totalQuestionAttempted) * 100)
    /**
    ...
    const totalRemaningQuesPercentage = parseInt((totalQuesCount / total) * 100)
    const remaingAllWrongPercentage = parseInt(((total - totalQuestionCorrect) / total) * 100)
  ...
  render () {
    const {
      nr, total, question, category, answers, correct, showButton, questionAnswered, remaingAllCorrectPercentage, displayPopup, score, classNames, isAnswerCorrect,
      currentScorePercentage, remaingAllWrongPercentage, difficulty, quesPercentage
    } = this.state

    return (
      <div className='container'>

        <Popup style={{ display: displayPopup }} score={score} total={this.state.total} onStartQuiz={this.handleStartQuiz} />

        <div className='row'>

          <div className='col-lg-10 col-lg-offset-1'>
            <ProgressBar className='Progresscurrent' variant='info' now={quesPercentage} label={`${quesPercentage}%`} />
            <div id='question'>
            ...
            <Answers answers={answers} correct={correct} onShowButton={this.handleShowButton} isAnswered={questionAnswered} onIncreaseScore={this.handleCorrectAttemp} onWrongAttemp={this.handleWrongAttemp} onHandlePercentage={this.handlePercentage} classNames={classNames} />
            <div className='isanswercorrect'>
              {showButton ? isAnswerCorrect ? 'Correct!' : 'Sorry!' : null}
            </div>

            <div id='submit'>
              {showButton ? <button className='fancy-btn' onClick={this.handleNextQuestion}>{nr === total ? 'Finish quiz' : 'Next question'}</button> : null}
            </div>
            <span>Score: {currentScorePercentage}%</span>
            <span className='maxScore'> Max Score: {remaingAllCorrectPercentage}%</span>
            <ProgressBar className='progressStyle'>
              <ProgressBar className='Progresscurrent' striped variant='success' now={currentScorePercentage} label={`${currentScorePercentage}%`} key={1} />
              <ProgressBar className='Progressmax' variant='warning' now={remaingAllCorrectPercentage} label={`${remaingAllCorrectPercentage}%`} key={2} />
              <ProgressBar className='Progressmin' striped variant='danger' now={remaingAllWrongPercentage} label={`${remaingAllWrongPercentage}%`} key={3} />
              <ProgressBar className='progressbase' variant='info' now={100} key={4} />
            </ProgressBar>
